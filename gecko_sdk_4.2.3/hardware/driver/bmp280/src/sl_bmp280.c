...
#define SL_BMP280_DEVICE_ID                   0x60   /**< Device ID of the BMP280 chip          */
#define SL_BMP280_I2C_BUS_ADDRESS             0x76   /**< I2C address of the BMP280 chip        */
...
/***************************************************************************//**
 *    Initiates humidity measurement on the barometric pressure sensor and
 *    reads humidity from it.
 ******************************************************************************/
sl_status_t sl_bmp280_measure_humidity(sl_i2cspm_t *i2cspm, float *humidity)
{
  int8_t result;
  int32_t uncompHum;
  int32_t compHum;
  ...
  bmp280_i2cspm_instance = i2cspm;
    ...
    result = bmp280_read_uncomp_humidity(&uncompHum);

  if (result != SUCCESS) {
    return SL_STATUS_FAIL;
  }

  compHum = bmp280_compensate_humidity_int32(uncompHum);

  *humidity = (float)compHum;
  *humidity /= 1024.0f;
